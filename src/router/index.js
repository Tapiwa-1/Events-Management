...
const routes = [
  ...
  { path: '/register', component: () => import('../views/auth/Register.vue'), meta: { guest: true } },
  {
    ...
    children: [
      ...
      { path: 'packages', component: () => import('../views/PackagesManagement.vue'), meta: { role: 'admin' } },
      { path: 'logistics', component: () => import('../views/Logistics.vue'), meta: { role: 'staff' } },
      { path: 'profile', component: () => import('../views/Profile.vue') },
      { path: 'logs', component: () => import('../views/admin/AuditLogs.vue'), meta: { role: 'admin' } },
    ]
  },
];
...
router.beforeEach(async (to, from, next) => {
  ...
  if (to.meta.role) {
    ...
    // Simple role check
    if (role === 'admin' && !authStore.isAdmin) return next('/admin');
    if (role === 'staff' && !authStore.isStaff) return next('/admin');
  }
